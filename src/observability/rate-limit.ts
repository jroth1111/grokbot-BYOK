  ...
  for (const value of error.details) {
    ...
    switch (reason) {
      ...
      case 'RATE_LIMIT_EXCEEDED': {
        const retryDelayMs = extractRetryHint(errorMessage);
        return retryDelayMs !== null && retryDelayMs >= LONG_RATE_LIMIT_DELAY_MS
          ? 'QUOTA_EXHAUSTED'
          : 'RATE_LIMIT_EXCEEDED';
      }
    }
 ...
 */
export function classifyRateLimitReason(errorMessage: string): RateLimitReason {
 ...
 */
export function backoffForRateLimitReason(reason: RateLimitReason): number {
  switch (reason) {
    case 'INSUFFICIENT_G1_CREDITS_BALANCE':
    case 'QUOTA_EXHAUSTED':
...
/**
 * Simplified inline usage-limit check (replaces the `isUsageLimit` flag accessor
 * from `./flags`). Returns true when the message carries persistent account
 * quota / billing-cap phrasing that should rotate to a sibling credential.
 */
export function isUsageLimit(errorMessage: string): boolean {
  const structuredReason = parseGoogleRpcRateLimitReason(errorMessage);
 ...
 *
 *  1. Body matches {@link isUsageLimit} (Codex `usage_limit_reached`,
 ...
 *  4. Body has content → defer to {@link classifyRateLimitReason}. `QUOTA_EXHAUSTED`
...
export function isUsageLimitOutcome(status: number | undefined, message: string | undefined): boolean {
  ...
  if (message && isUsageLimit(message)) return true;
  ...
  const reason = classifyRateLimitReason(message);
...
export function isOpaqueStatusBody(message: string): boolean {
  ...
  // exhaustion or a throttle) defers to classifyRateLimitReason; a body that
 ...
 * Kept separate from {@link isUsageLimit} because the bare wording is
...
export function isConcurrencyCapExclusion(status: number | undefined, message: string | undefined): boolean {
  return message !== undefined && classifyRateLimitReason(message) === 'CONCURRENT_LIMIT' && status !== 402;
}
...
/**
 * Inline server-suggested retry delay extraction. Extracts a retry hint
 * (seconds or duration) from error message bodies — "try again in X seconds",
 * "retry after N", "reset in X minutes" — and returns it as milliseconds, or
 * `null` when no signal is found. Replaces the `extractRetryHint` helper from
 * `@oh-my-pi/pi-utils` (body-pattern portion only; header sources are not
 * available at this classification layer).
 */
export function extractRetryHint(message: string): number | null {
  if (!message) return null;

  const quotaMatch = QUOTA_RESET_PATTERN.exec(message);
  if (quotaMatch) {
