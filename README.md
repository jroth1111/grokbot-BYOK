...
through Cursor's paid backend. This project installs a local proxy that
intercepts those requests and routes them to any OpenAI-compatible LLM
provider you choose: OpenRouter, OpenCode, Kilo, a local model server, or
all of the above with automatic failover.

You bring the keys (BYOK = Bring Your Own Key). The shim handles the rest:
protocol translation, routing, failover, and automatic re-patching when
Cursor updates itself.
...
Cursor's editor talks to its inference backend using Connect-RPC, a
...
the wire format is different. Cursor sends Connect. Providers expect
OpenAI SSE.
...
The shim:
1. **Receives** Connect-RPC streaming requests from Cursor
...
6. **Streams** the response back to Cursor

Cursor doesn't know the proxy is there. It thinks it's talking to its
normal backend. The shim patches Cursor's host bundle (`host-main.cjs`) to
redirect inference calls to `localhost:8788`, and a background watcher
re-applies the patch automatically whenever Cursor updates itself.
...
- Node.js 18+
- Cursor installed (the editor, not the database library)
...
# 2. One-command setup (installs deps, builds, patches Cursor, starts daemons)
...
Cursor's AI features now route through your providers.

### Verifying it works

```bash
npm status                 # Check daemon status (all three should be "running")
tail -20 /tmp/inference-shim.log  # Check the shim log for errors
node dist/scripts/live-test.js    # Send a test request through the shim
```

If `npm status` shows all daemons running and the live test returns text,
the shim is working. If something failed, check
[Troubleshooting](#troubleshooting).
...
5. Builds the shim + scripts (esbuild to standalone bundles)
6. Starts three background daemons (see [Managing daemons](#managing-daemons))
7. Patches Cursor's `host-main.cjs` to redirect inference to the shim
8. Runs a smoke test
...
- Replace the `createCursorInferencePromptSession` function with a
  ...
  shim (reading the proxy URL from `inference-proxy.url`)
...
Cursor updates its own `host-main.cjs` periodically (on restart or
update). A background watcher daemon monitors the file and re-applies the
patch automatically when it changes. The shim keeps working across Cursor
...
| `SAND_HOST_DIR` | `$HOME/sand-host` | Cursor sand-host directory (where host-main.cjs lives) |
...
> have WindsurfAPI installed, the shim still works with the other
...
|--------|----------|----------|---------|
| Shim | `/tmp/inference-shim.pid` | `/tmp/inference-shim.log` | The inference proxy itself (port 8788) |
...
The host watcher is what makes re-attachment robust. When Cursor's
...
This happens automatically. No manual intervention needed after Cursor
updates.

The health watchdog parses the shim's log to compute an error rate over a
...
completion), the shim fails over to the next provider instead of returning
...
When `routingStrategy: "latency"`, the shim maintains per-provider
...
When `sessionAffinity.enabled: true`, the shim binds a session ID (derived
...
vision, the shim re-routes to `VISION_FALLBACK_MODEL` (default:
`qwen3.8-max`) instead of silently stripping the images. If no fallback
model is configured, the shim replaces images with a placeholder text and
...
The shim handles tool calls across the Connect to OpenAI translation
boundary:
...
- **Inline tool-call rescue.** When a model emits tool calls as text in a
  ...
  `<tool_calls>` XML), the shim detects and re-parses them into structured
  tool calls.
...
The shim logs per-provider metrics every 5 minutes:
...
The health watchdog calls this script automatically when the shim's error
...
- **Cursor not found.** The setup script looks for `host-main.cjs` in
  `$HOME/sand-host`. If your Cursor installation uses a different location,
  set `SAND_HOST_DIR` in `.env`.

### Cursor's AI features stopped working
...
If the shim crashed, the health watchdog should auto-redeploy within 5
minutes. If it didn't, run `npm run setup` to restart everything.

If the host patch was lost (Cursor updated and the watcher didn't catch
...
### Shim is running but Cursor doesn't use it
...
- End-to-end (real shim server, real HTTP requests)
...
```bash
npm run build         # Build shim only (dist/shim.js)
npm run build:all     # Build shim + scripts (dist/shim.js + dist/scripts/*.js)
npm run clean         # Remove dist/
```

The build uses esbuild to produce standalone bundled `.js` files with no
runtime dependencies (except `ajv` and `zod`, which are bundled in). The
shim is a single `dist/shim.js` file that can run anywhere Node 18+ is
available.
...
scripts/
  ...
  start-shim.ts                 Shim launcher (WindsurfAPI auto-start)
